...
if [ ! -d output/tmp ]; then
    ...
    sudo mkdir -p $output/usr/src/gsmenu
    sudo mount -o bind $(pwd) $output/usr/src/gsmenu
fi
...
if [ ! -f $output/tmp/prepare_chroot.done ]; then
    ...
    apt-get update
    apt-get install -y git g++ gcc make pkg-config libspdlog-dev libevent-dev libcairo-dev libdrm-dev cmake libgpiod-dev
    apt clean
